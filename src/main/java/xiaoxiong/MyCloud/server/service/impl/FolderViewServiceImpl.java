...
import xiaoxiong.MyCloud.server.pojo.FolderView;
import xiaoxiong.MyCloud.server.pojo.SreachView;
...
public class FolderViewServiceImpl implements FolderViewService{
	...
	public String getFolderViewToJson(final String fid, final HttpSession session, final HttpServletRequest request) {
		...
		fv.setFolderList(fs);//fm.queryByParentId(fid)可查询出以当前文件夹下为父文件夹的所有文件夹
		fv.setFileList(this.flm.queryByParentFolderId(fid)); //文件夹下的所有文件
		if (account != null) {
